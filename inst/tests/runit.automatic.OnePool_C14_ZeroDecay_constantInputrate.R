...
test.OnePool_C14_ZeroDecay_constantInputrate_c14=function(){
   require(RUnit)
   t_start=0
   t_end=2
   tn=100
 ...
 format="AbsoluteFractionModern")
   ...
   Y14[,1]=c01*f01*exp(-t*log(2)/5730) + 2865.0/log(2) - 2865.0*exp(-t*log(2)/5730)/log(2)
   F14=matrix(ncol=1,nrow=length(t))
   F14[,1]=-1000 + 1000*(c01*f01*exp(-t*log(2)/5730) + 2865.0/log(2) - 2865.0*exp(-t*log(2)/5730)/log(2))/(c01 + t)
   mod=GeneralModel_14(
    t,
    A,
    c(
...
#begin plots 
   lt1=2
   lt2=4
   pdf(file="runit.automatic.OnePool_C14_ZeroDecay_constantInputrate_c14.pdf",paper="a4")
