...
test.TwopSeriell=function(){
   ...
   Y[,1]=c01*exp(-t/20)
   Y[,2]=c01*(-9*exp(-t/20)/4 + 9*exp(-t/30)/4) + c02*exp(-t/30)
   R=matrix(ncol=2,nrow=length(t))
   R[,1]=c01*exp(-t/20)/80
   R[,2]=c01*(-9*exp(-t/20)/4 + 9*exp(-t/30)/4)/30 + c02*exp(-t/30)/30
   mod=GeneralModel(
    t,
    A,
    c(
...
#begin plots 
   lt1=2
   lt2=4
   pdf(file="runit.automatic.TwopSeriell.pdf",paper="a4")
